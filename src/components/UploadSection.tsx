...
export default function UploadSection() {
  ...
  const createPlaylist = async () => {
    ...
    // Ask the user for a playlist name (with a sensible default)
    const defaultName = `MusicBuzz – ${new Date().toLocaleString()}`;
    const name = window.prompt("Name your playlist:", defaultName);
    ...
    try {
      const res = await fetch(`/api/spotify/playlist`, {
        ...
        body: JSON.stringify({
          ...
          uris: playableUris,
          isPublic: false       // optional: later you can expose a toggle
        }),
      });
